...
struct MCMCResult {std::vector<double> E_samples; std::vector<double> M_samples; int sweeps_done = 0; long long accepted_flips = 0;};

MCMCResult run_mcmc_metropolis(std::vector<std::vector<int>>& s, int L, double T, double J = 1.0,
    int total_sweeps = 5000,
    int burn_in = 1000,
    int sample_every = 10,
